...
if not setfenv then -- Lua 5.2+; this assumes f is a function
   ...
   setfenv = function(f, t)
      ...
      if level then
         debug.upvaluejoin(f, level, function()
            return t
         end, 1)
      end
      return f
...
local LogDebug = function(...)
   return Log(kLogDebug, logFormat(...))
end
local LogVerbose = function(...)
   return Log(kLogVerbose, logFormat(...))
end
local LogInfo = function(...)
   return Log(kLogInfo, logFormat(...))
end
local LogWarn = function(...)
   return Log(kLogWarn, logFormat(...))
end
local istype = function(b)
   return function(mode)
      return math.floor((mode % (2 * b)) / b) == 1
   end
end
...
local function reg2x(v)
   return reg("x2", v)
end
local function reg1p(v)
   return reg("p1", v)
end
local getTimeNano = (
   unix and function()
...
local function obsolete(obj, old, new, ver)
   obj[old] = VERSION < ver
         and function(...)
            LogWarn(("method %s has been replaced by %s and will be removed in v%s."):format(old, new, ver))
            obj[old] = obj[new]
            return obj[new](...)
         end
      or nil
...
local function makePath(name, params)
   ...
   route = route:gsub(PARAM .. "([^(*:]*)", function(sigil, param, rest)
      if sigil == "*" and param == "" then
         param = "splat"
      end
      -- ignore everything that doesn't match `:%w` pattern
      if sigil == ":" and param == "" then
         return sigil .. param .. rest
      end
      -- if the parameter value is `false`, replace it with an empty string
      return ((params[param] or (params[param] == false and "" or sigil .. param)) .. rest:gsub("^%b[]", ""))
   end)
   ...
   argerror(not param, 2, "(missing required parameter " .. (param and #param > 0 and param or "splat") .. ")")
   return route
end
local function makeUrl(url, opts)
   if type(url) == "table" and opts == nil then
      url, opts = nil, url
   end
   if not url then
      url = GetUrl()
...
local function genEnv(opt)
   ...
   return function(t, key)
      ...
      if not opt.request and val == nil then
         val = _G[key]
      end
   ...
   local function optignorecase(s)
      if not patts[s] then
         patts[s] = (
            ";%s*"
            .. s:gsub("%w", function(s)
               return ("[%s%s]"):format(s:upper(), s:lower())
            end)
            .. [[=["']?([^;"']*)["']?]]
         )
      end
...
local function parseMultipart(body, ctype)
   ...
   while true do
      repeat
         ...
         if not eol then
            return nil, "missing expected boundary at position " .. bol
         end
      ...
      if bol > 1 then
         ...
         if string.sub(body, bol, bol + CRLFlen - 1) == CRLF then -- no headers
            ...
            if not eoh then
               return nil, "missing expected end of headers at position " .. bol
            end
            ...
            while (string.find(head, CRLF, boh, true) or 0) > boh do
               ...
               if p ~= boh then
                  return nil, "invalid header syntax at position " .. bol + boh
               end
               header = header:lower()
      ...
      if tail ~= CRLF then
         return nil, "missing closing boundary at position " .. eol
      end
      bol = eob + #tail + 1
   end
   return parts
...
local function render(name, opt)
   ...
   for k, v in pairs(rawget(env, org) or {}) do
      params[k] = v
   end
...
local function setTemplate(name, code, opt)
   ...
   if type(code) == "table" then
      params, code = code, table.remove(code, 1)
   end
   ...
   local env = setmetatable(
      { render = render, [org] = opt },
      -- get the metatable from the template that this one is based on,
      -- to make sure the correct environment is being served
      tmpl and getmetatable(getfenv(tmpl.handler))
         or params.autotag and tmplTagHandlerEnv
         or tmplRegHandlerEnv
   )
   if ctype == "string" then
      ...
      if tmpl.autotag then
         tmpl.parser = setfenv(tmpl.parser, env)
      end
...
local function setTemplateVar(name, value)
   vars[name] = value
end
...
local setmap = {};
(function(s)
   for pat, reg in s:gmatch("(%S+)=([^%s,]+),?") do
      setmap[pat] = reg
   end
end)([[
...
local function route2regex(route)
   ...
   local params = {}
   local regex = route
      :gsub("%)", "%1?") -- update optional groups from () to ()?
      :gsub("%.", "\\.") -- escape dots (.)
      ...
      :gsub("%b[](%+%))(%b[])([^/:*%[]*)", function(sep, pat, rest)
         ...
         return pat:gsub("%%(.)", findset) .. sep .. rest
      end)
   ...
   while true do
      ...
      if capture > "" then
         table.insert(params, subnum, false)
      end
      subnum = subnum + 1
   end
   return "^" .. regex .. "$", params
...
local function findRoute(route, opts)
   for i, r in ipairs(routes) do
      ...
      if
         route == r.route
         and (type(ometh) == "table" and table.concat(ometh, ",") or ometh)
            == (type(rmeth) == "table" and table.concat(rmeth, ",") or rmeth)
      then
         return i
      end
   end
...
local function setRoute(opts, ...)
   ...
   argerror(
      ht == "function" or ht == "string" or (ht == "nil" and pnum == 0),
      2,
      "(function or string expected)",
      "setRoute"
   )
   if ht == "string" then
   ...
   if ot == "table" then
      ...
      for k, v in pairs(opts) do
         if type(v) == "table" then
            ...
            if v.regex then
               v.regex = argerror(re.compile(v.regex), 1, ("(valid regex expected for '%s')"):format(k), "setRoute")
            end
   ...
   while true do
      ...
      if opts.routeName then
         if routes[opts.routeName] then
            LogWarn("route '%s' already registered", opts.routeName)
         end
...
local function matchCondition(value, cond)
   ...
   if value == nil or cond[value] then
      return true
   end
   if cond.regex then
      return cond.regex:search(value) ~= nil
   end
   if cond.pattern then
      return value:match(cond.pattern) ~= nil
   end
   return false
end
...
local function getAllowedMethod(matchedRoutes)
   ...
   table.sort(methods)
   return (
      #methods > 0 and table.concat(methods, ", ") .. (methods.OPTIONS == nil and ", OPTIONS" or "")
      ...
      if route.handler or opts and opts.otherwise then
         ...
         if matched then -- path matched
            ...
            for ind, val in ipairs(route.params) do
               if val and res[ind] then
                  req.params[val] = res[ind] > "" and res[ind] or false
               end
            end
               ...
               for filter, cond in pairs(opts) do
                  if filter ~= "otherwise" then
                     ...
                     if not resCond then
                        ...
                        LogDebug(
                           "route '%s' filter '%s%s' didn't match value '%s'%s",
                           route.route,
                           filter,
                           type(cond) == "string" and "=" .. cond or "",
                           value,
                           tonumber(otherwise) and " and returned " .. otherwise or ""
                        )
                        if otherwise then
...
local dbmt = { -- share one metatable among all DBM objects
   ...
   __index = function(t, k)
      ...
      return db and db[k] and function(self, ...)
         return db[k](db, ...)
      end or nil
   end,
   __gc = function(t)
...
local function makeStorage(dbname, sqlsetup, opts)
   ...
   for flagname, val in pairs(opts or {}) do
      local flagcode = flagname:find("^OPEN_") and (sqlite3[flagname] or error("unknown option " .. flagname))
      flags = flags | (val and flagcode or 0)
   end
   argerror(
      not opts or not opts.trace or type(opts.trace) == "function",
      3,
      "(function expected as trace option value)"
   )
   ...
   local dbm = { NONE = NONE, prepcache = {}, pragmas = {}, name = dbname, sql = sqlsetup, opts = opts or {} }
   local msgdelete = "use delete option to force"
   local function getPragmas(sql)
      local pragmas = {}
      for p in (sql or ""):gmatch("%s*([^;]+)") do
         if not p:lower():find("pragma ") then
            break
         end
   ...
   function dbm:init(reopen)
      ...
      if not db then
         error(("%s (code: %d)"):format(msg, code))
      end
      ...
      if debug.getmetatable(db) then
         debug.getmetatable(db).__gc = nil
      end
      ...
      if pragmas and db:exec(pragmas) > 0 or not skipexec and self.sql and db:exec(self.sql) > 0 then
   ...
   local function prepstmt(dbm, stmt)
      if not dbm.prepcache[stmt] then
         ...
         if st and tail and #tail > 0 then
            return st, tail
         end
   ...
   local function fetch(self, query, one, ...)
      ...
      repeat
         ...
         if not stmt then
            return nil, "can't prepare: " .. self.db:errmsg()
         end
         -- if the last statement is incomplete
         if not stmt:isopen() then
            break
         end
      ...
      if trace then
         trace(self, query, { ... }, getTimeDiff(start))
      end
      if one == nil then
         return self.db:changes()
      end -- return execute results
   ...
   function dbm:execute(list, ...)
      ...
      local db = self.db
      local changes = 0
      if db:exec("savepoint execute") ~= sqlite3.OK then
         return dberr(db)
      end
      for _, sql in ipairs(list) do
         ...
         if not ok then
            if db:exec("rollback to execute") ~= sqlite3.OK then
               return dberr(db)
            end
            return nil, err
         end
      ...
      if db:exec("release execute") ~= sqlite3.OK then
         return dberr(db)
      end
      return changes
   end
   function dbm:fetchAll(stmt, ...)
      return fetch(dbm, stmt, false, ...)
   end
   function dbm:fetchOne(stmt, ...)
      return fetch(dbm, stmt, true, ...)
   end
   function dbm:pragma(stmt)
      local pragma = stmt:match("[_%w]+")
      if not self.pragmas[pragma] then
         if self:fetchOne("select * from pragma_pragma_list() where name = ?", pragma or "") == self.NONE then
            return nil, "missing or invalid pragma name"
         end
   ...
   function dbm:upgrade(opts)
      ...
      local actbl, prtbl = {}, {}
      for r in pristine:nrows(sqltbl) do
         prtbl[r.name] = r.sql
      end
      for r in actual:nrows(sqltbl) do
         ...
         if prtbl[r.name] then
            if norm(r.sql) ~= norm(prtbl[r.name]) then
               ...
               local common, prcol = {}, {}
               for c in pristine:nrows(sqlcol) do
                  prcol[c.name] = true
               end
               for c in actual:nrows(sqlcol) do
                  ...
                  elseif not opts.delete then
                     err = err or ("Not allowed to remove '%s' from '%s'; %s"):format(c.name, r.name, msgdelete)
                  end
               end
               local cols = table.concat(common, ",")
               table.insert(changes, ("INSERT INTO %s (%s) SELECT %s FROM %s"):format(tmpname, cols, cols, r.name))
            ...
            if opts.delete == nil then
               err = err or ("Not allowed to drop table '%s'; %s"):format(r.name, msgdelete)
            end
      ...
      if opts.integritycheck ~= false then
         ...
         if fkc and fkc ~= self.NONE then
            return nil, "foreign key check failed"
         end
      end
      if opts.dryrun then
         return changes
      ...
      if self:pragma(pfk) and ok then
         table.insert(changes, "PRAGMA " .. pfk)
      end
      if not ok then
         return ok, err
      end
...
local function setHook(name, func)
   ...
   if not func then
      ...
      if idx and isQualified then
         table.remove(hooks[main], idx)
      end
   ...
   for _, v in pairs(schedules) do
      ...
      if cront[1][m] and cront[2][h] and cront[3][dom] and cront[4][mon] and cront[5][dow] then
         if sameproc or assert(unix.fork()) == 0 then
            ...
            if not ok then
               LogWarn("scheduled task failed: " .. err)
            end
            if not sameproc then
               unix.exit(0)
            end
         end
      end
...
local function setSchedule(exp, func, opts)
   ...
   if not setHook(scheduleHook, scheduler) then -- first schedule hook
      if ProgramHeartbeatInterval then
         local min = 60 * 1000
         if ProgramHeartbeatInterval() > min then
            ProgramHeartbeatInterval(min)
         end
      else
         LogWarn(
            "OnServerHeartbeat is required for setSchedule to work,"
...
local function makeLastModified(asset)
   ...
   return {
      function(ifModifiedSince)
         local isModified = (not ifModifiedSince or ParseHttpDateTime(ifModifiedSince) < lastModified)
         if isModified then
...
local trueval = function()
   return true
end
local validators = {
   msg = trueval,
   optional = trueval,
   minlen = function(s, num)
      return #tostring(s or "") >= num, "%s is shorter than " .. num .. " chars"
   end,
   maxlen = function(s, num)
      return #tostring(s or "") <= num, "%s is longer than " .. num .. " chars"
   end,
   pattern = function(s, pat)
      return tostring(s or ""):match(pat), "invalid %s format"
   end,
   test = function(s, fun)
      return fun(s)
   end,
...
local function makeValidator(rules)
   ...
   for i, rule in ipairs(rules) do
      ...
      argerror(
         not rule.test or type(rule.test) == "function",
         1,
         "(rule with test as function expected at position " .. i .. ")"
      )
   end
   return setmetatable({
      function(val)
         ...
         for _, rule in ipairs(rules) do
            repeat
               ...
               if value == nil and rule.optional == true then
                  break
               end -- continue
               for checkname, checkval in pairs(rule) do
                  if type(checkname) == "string" then
                     ...
                     if not validator then
                        argerror(false, 1, "unknown validator " .. checkname)
                     end
         ...
         if #errors > 0 or next(errors) then
            return nil, errors
         end
         return true
      end,
...
local function makeBasicAuth(authtable, opts)
   ...
   return {
      function(authorization)
         ...
         if not pass or not user or not authtable[user] then
            return false
         end
         if hash:upper() == "ARGON2" then
            return argon2.verify(authtable[user], pass)
         end
...
local function makeIpMatcher(list)
   ...
   return function(ip)
      if ip == -1 then
         return false
      end -- fail the check on invalid IP
...
local function error2tmpl(status, reason, message)
   ...
   local ok, res = pcall(render, tostring(status), { status = status, reason = reason, message = message })
...
local function checkPath(path)
   return type(path) == "string" and path or GetPath()
end
local fm = setmetatable({
   ...
   reg1p = reg1p,
   getBrand = function()
      return ("%s/%s %s/%s"):format("redbean", getRBVersion(), NAME, VERSION)
   end,
   ...
   serveIndex = function(path)
      return function()
         return ServeIndex(checkPath(path))
      end
   end,
   -- handle and serve existing path, including asset, Lua, folder/index, and pre-configured redirect
   servePath = function(path)
      return function()
         return RoutePath(checkPath(path))
      end
   end,
   -- return asset (de/compressed) along with checking for asset range and last/not-modified
   serveAsset = function(path)
      return function()
         return ServeAsset(checkPath(path))
      end
   end,
   serveError = function(status, reason, msg)
      return function()
         return error2tmpl(status, reason, msg)
      end
   end,
   serveContent = function(tmpl, params)
      return function()
         return render(tmpl, params)
      end
   end,
   serveRedirect = function(status, loc)
      return function()
         ...
         return ServeRedirect(status or loc and 307 or 303, loc or GetPath())
      end
   end,
   ...
   __index = function(t, key)
      ...
      if serveStatus then
         return cache(t.serveError(tonumber(serveStatus)))
      end
      -- handle logVerbose and other log calls
      local kVal = not _G[key] and _G[key:gsub("^l(og%w*)$", function(name)
         return "kL" .. name
      end)]
      if kVal then
         return cache(function(...)
            return Log(kVal, logFormat(...))
         end)
      end
...
local function getSessionOptions()
   ...
   if not sopts.name then
      error("missing session name")
   end
   if not sopts.hash then
      error("missing session hash")
   end
   if not sopts.format then
      error("missing session format")
   end
...
local function setSession(session)
   ...
   if session and session[isfresh] then
      return
   end
   ...
   if session and next(session) then
      ...
      local sig = EncodeBase64(GetCryptoHash(sopts.hash, msg, sopts.secret or ""))
...
local function getSession()
   ...
   if not ok then
      LogWarn("invalid session content: " .. val)
   end
...
local function handleRequest(path)
   ...
   req = setmetatable({
      params = setmetatable({}, {
         __index = function(t, k)
            ...
            if not HasParam(k) and HasParam(mk) then
               k = mk
            end
            ...
            if not string.find(k, MULTIVAL) then
               return GetParam(k) or false
            end
            local array = {}
            for _, v in ipairs(GetParams()) do
               if v[1] == k then
                  table.insert(array, v[2] or false)
               end
            end
            return array
         end,
      ...
      headers = setmetatable({}, {
         __index = function(_, k)
            return GetHeader(headerMap[k] or k)
         end,
      }),
      ...
      session = setmetatable({ [isfresh] = true }, {
         __index = function(t, k)
            if t[isfresh] == true then
               t[isfresh] = getSession()
            end
   ...
   elseif tres == "string" then
      if #res > 0 then
         if not headers then
            headers = { ContentType = detectType(res) }
         end
...
local function streamWrap(func)
   return function(...)
      return coroutine.yield(func(...)()) or true
   end
end
...
function fm.run(opts)
   ...
   if logpath then
      ProgramLogPath(logpath)
   end
   for key, v in pairs(opts) do
      if key == "headers" and type(v) == "table" then
         for h, val in pairs(v) do
            ProgramHeader(headerMap[h] or h, val)
         end
   ...
   if GetLogLevel then
      ...
      if level < kLogWarn then
         LogWarn = none
      end
      if level < kLogInfo then
         LogInfo = none
      end
      if level < kLogVerbose then
         LogVerbose = none
      end
      if level < kLogDebug then
         LogDebug = none
      end
   end
   ...
   if sopts.secret == true then
      ...
      LogVerbose(
         "applied random session secret; set `fm.sessionOptions.secret`"
            .. (" to `fm.decodeBase64('%s')` to continue using this value"):format(EncodeBase64(sopts.secret))
...
fm.setTemplate("fmt", {
   parser = function(tmpl)
      ...
      local function writer(s)
         return #s > 0 and ("Write(%q)"):format(s) or ""
      end
      local function decomment(s)
         return s:match("%[=*%[") and "--" .. s or ""
      end
         ...
         return writer(htm)
            .. (
               val ~= EOT -- this is not the suffix
                  and (
                     pref == "" -- this is a code fragment
                        and val:gsub("%-%-([^\r\n]*)", decomment) .. " "
                     or ("Write(%s(tostring(%s or '')))"):format(
                        pref == "&" and "escapeHtml" or "",
                        val:gsub("%-%-(.*)", decomment)
                     )
                  )
               or ""
...
fm.setTemplate("json", {
   ...
   function(val)
      return EncodeJson(val, { useoutput = true })
   end,
})
...
local function fmgRender(val)
   ...
   local function writeVal(opt, escape)
      ...
      if type(opt) == "table" then
         ...
         if tag == "include" then
            return (fm.render(opt[2], opt[3]))
         end
         if tag == "raw" then
            for i = 2, #opt do
               writeVal(opt[i], false)
            end
            return
         end
         if tag == "each" then
            ...
            for _, v in ipairs(opt[3]) do
               writeVal(opt[2](v), false)
            end
            return
         end
...
fm.setTemplate("cgi", function(cmd)
   ...
   local envd = #envu > 0 and envu
      or {
         ...
         REQUEST_URI = (function(u)
            return EncodeUrl({ params = u.params, path = u.path })
         end)(ParseUrl(GetUrl())),
   ...
   if #envu == 0 then
      ...
      for k, v in pairs(GetHeaders()) do
         envd["HTTP_" .. k:upper()] = v
      end
      -- overwrite all defaults
      for k, v in pairs(envu) do
         envd[k] = v
      end
      -- convert to strings unless the value is false
      for k, v in pairs(envd) do
         if v then
   ...
   if pid == 0 then -- forked child
      ...
      local header = true
      local done = false
      assert(unix.sigaction(unix.SIGALRM, function()
         isactive = false
      end))
      ...
      while true do
         ...
         local data, errno
         if se & unix.POLLIN > 0 then
            data, errno = unix.read(rfd1)
         end
         -- check for end of file or any descriptor errors
         if data == "" or errno and errno:errno() == unix.EBADF then
            break
         end
      ...
      if not done then
         assert(unix.kill(pid, unix.SIGTERM))
      end
   end
end)
